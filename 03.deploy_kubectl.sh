...
do
    echo ">>> $host"
    scp $PWD/bin/kubectl $USERNAME@$host:$BASE_DIR/bin
    ssh $USERNAME@$host "chmod +x $BASE_DIR/bin/*"
done

#sh $PWD/scripts/kubectl/create_admin_csr.sh
#################################################################
...
#################################################################
function create_csr(){
cat > $BASE_DIR/work/admin-csr.json <<EOF
{
  "CN": "admin",
  "hosts": [],
  "key": {
...
do
    echo ">>> $host"
    ssh $USERNAME@$host "mkdir -p ~/.kube"
    scp kubectl.kubeconfig  $USERNAME@$host:~/.kube/config
